...
inline static node_id_t extract_left_bits(node_id_t number, size_t pos) { return number >> pos; }
...
void PipelineHyperTree::print_r_to_l(node_id_t src) {
  std::cout << "src: " << src;
  std::cout << "->(L1)" << extract_left_bits(src, level1_pos);
  std::cout << "->(L2)" << extract_left_bits(src, level2_pos);
  if (level3_gutters) std::cout << "->(L3)" << extract_left_bits(src, level3_pos);
  if (level4_gutters) std::cout << "->(L4)" << extract_left_bits(src, level4_pos);
  std::cout << std::endl;
}
...
PipelineHyperTree::~PipelineHyperTree() {
...
void PipelineHyperTree::InsertThread::insert(update_t upd) {
...
void PipelineHyperTree::InsertThread::batch_insert(const update_t *batch, size_t num_updates) {
...
void PipelineHyperTree::InsertThread::process_stream_upd_batch(const GraphStreamUpdate *upds,
                                                               size_t num_updates) {
  for (size_t i = 0; i < num_updates; i++) {
...
void PipelineHyperTree::InsertThread::flush_l1_buf(node_id_t buf_idx) {
...
void PipelineHyperTree::InsertThread::flush_l2_buf(node_id_t buf_idx) {
...
void PipelineHyperTree::InsertThread::batch_shared_insert(SharedGutter **gutters,
                                                          const size_t buf_idx,
                                                          std::vector<update_t> &updates) {
  SharedGutter *gutter = gutters[buf_idx];
  gutter
      ->active_inserts++;  // avoids race cases. Forces threads to pick a safe instruction ordering
  bool done = false;
  while (!done) {
...
void PipelineHyperTree::InsertThread::batch_leaf_insert(const size_t buf_idx,
...
bool PipelineHyperTree::SharedGutter::batch_insert(InsertThread &thr, SharedGutter *&gut_ptr,
                                                   const std::vector<update_t> &updates) {
  // std::cerr << "SharedGutter " << index << " batch_insert(" << updates.size() << ") " <<
  // std::endl;
  active_inserts++;
...
void PipelineHyperTree::SharedGutter::flush(InsertThread &thr, SharedGutter *&gut_ptr, size_t pos,
                                            const std::vector<update_t> &updates) {
...
bool PipelineHyperTree::LeafGutter::batch_insert(InsertThread &thr, LeafGutter *&gut_ptr,
...
void PipelineHyperTree::LeafGutter::flush(InsertThread &thr, LeafGutter *&gut_ptr, size_t pos,
                                          const std::vector<node_id_t> &updates) {
...
void PipelineHyperTree::InsertThread::wq_push_helper(node_id_t node_idx, LeafGutter &leaf,
                                                     size_t exp_size) {
  ...
  if (local_wq_buffer.size >= CGsystem.wq_batch_per_elm) flush_wq_buf();
  leaf.insert_pos = 0;
}

void PipelineHyperTree::InsertThread::flush_wq_buf() {
...
void PipelineHyperTree::InsertThread::flush_all() {
  // flush root buffer
  batch_insert(root_buffer, root_buffer_size);
  root_buffer_size = 0;

#ifndef EXIT_LEVEL1
  for (size_t i = 0; i < level1_bufs; i++) flush_l1_buf(i);
#ifndef EXIT_LEVEL2
  for (size_t i = 0; i < level2_bufs; i++) flush_l2_buf(i);
#endif
#endif
}

void PipelineHyperTree::force_flush() {
  ...
  for (size_t i = 0; i < inserters; i++) threads.emplace_back(flush_task, i);

  for (size_t i = 0; i < inserters; i++) threads[i].join();
  ...
  for (size_t i = 0; i < inserters; i++) insert_threads[i].flush_wq_buf();
}
